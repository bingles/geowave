...
image::query.png[scaledwidth="100%",width="100%",alt="Query Architecture", title="Query Architecture"]
...
Queries are created in GeoWave through the use of query builders. These builders are used to set all the things needed to create a query, such as the type names, indices, authorizations, and query constraints. The base link:https://github.com/locationtech/geowave/blob/master/core/store/src/main/java/org/locationtech/geowave/core/store/api/QueryBuilder.java[`QueryBuilder`, window="_blank"] can be used as a general way to query any data. GeoWave also provides an implementation of the query builder that is specific to vector queries with the link:https://github.com/locationtech/geowave/blob/master/core/geotime/src/main/java/org/locationtech/geowave/core/geotime/store/query/api/VectorQueryBuilder.java[`VectorQueryBuilder`, window="_blank"]. It also provides a query builder for vector aggregation queries with the link:https://github.com/locationtech/geowave/blob/master/core/geotime/src/main/java/org/locationtech/geowave/core/geotime/store/query/api/VectorAggregationQueryBuilder.java[`VectorAggregationQueryBuilder`, window="_blank"]. These vector query builders provide a constraints factory that has additional constraints that are specific to vector queries, such as CQL filters.
...
Querying GeoWave using the GeoWave Query Language will return results in the form of a `ResultSet`, which is less like the results that would be obtained from a standard GeoWave query (SimpleFeatures) and more like the results that you would expect from querying a relational database (Rows).

==== Output Formats

New output formats are discovered using Service Provider Interface (SPI)-based injection. In order to install a new output format, implement link:https://github.com/locationtech/geowave/blob/master/extensions/adapters/vector/src/main/java/org/locationtech/geowave/adapter/vector/query/QueryOutputFormatSpi.java[`QueryOutputFormatSpi`, window="_blank"] and make sure your jar is on the classpath when running GeoWave. For more information on using SPI, see the link:https://docs.oracle.com/javase/tutorial/sound/SPI-intro.html[oracle documentation, window="_blank"].

==== Aggregation Functions

New aggregation functions can also be added to the query language using SPI. Aggregation functions should implement the link:https://github.com/locationtech/geowave/blob/master/extensions/adapters/vector/src/main/java/org/locationtech/geowave/adapter/vector/query/gwql/function/QLVectorAggregationFunction.java[`QLVectorAggregationFunction`, window="_blank"] interface and register it by implementing link:https://github.com/locationtech/geowave/blob/master/extensions/adapters/vector/src/main/java/org/locationtech/geowave/adapter/vector/query/gwql/function/QLFunctionRegistrySpi.java[`QLFunctionRegistrySpi`, window="_blank"].  Once both of these are implemented, make sure your jar is on the classpath when running GeoWave. For more information on using SPI, see the link:https://docs.oracle.com/javase/tutorial/sound/SPI-intro.html[oracle documentation, window="_blank"].