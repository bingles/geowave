...
image::architecture_overview.png[scaledwidth="100%",width="100%",alt="Operational Overview"]
...
* *Create Indices*
+
Create one or more multi-dimensional indices on the configured data store.
* *Ingest Data*
+
Ingest data into the specified store and indices.
* *Process*
+
Process data using a distributed processing engine (e.g. MapReduce, Spark).
* *Query*/*Discover*
+
Query or Discover ingested or transformed data using GeoWave operations. A common data discovery tool used is link:http://geoserver.org/[GeoServer^, window="_blank"], which interfaces with GeoWave through a plugin to visualize geospatial data in the underlying key/value store.
